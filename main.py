...
#from val_saveresults import validate_vis
...
from datasets.dataset_vis import ImageList_val_vis
...
from losses.ccc_ce_loss import CCC_CE_Loss
...
vis = configuration['visualization']
...
model_path = 'PretrainedWeights/TSAV_Sub4_544k.pth.tar' # path to the model
...
if flag == "Testing" or flag == "validation":
	cam_model_path = 'SavedWeights/cam_model_rec_2_v3.pt' # path to the model
...
#if vis:
#	valdataset = ImageList_val_vis(root=configuration['dataset_rootpath'], fileList=configuration['val_params']['labelpath'],
...
#	Valid_vacc, Valid_aacc = validate_vis(valloader, model, fusion_model)
#criterion = CCCLoss(digitize_num=1).cuda()
#criterion = CCC_CE_Loss(1).cuda()
criterion = CCCLoss(1).cuda()
#criterion2 = nn.MSELoss()

optimizer = torch.optim.AdamW(fusion_model.parameters(),# filter(lambda p: p.requires_grad, multimedia_model.parameters()),
								lr = configuration['model_params']['lr'], weight_decay = 0.00001)
...
for epoch in range(start_epoch, total_epoch):
	...
	if Valid_vacc > best_Val_acc:
		...
		if not os.path.isdir(path):
			os.mkdir(path)
		torch.save(state, os.path.join(path,'best_val_model.pt'))
		best_Val_acc = Valid_vacc
		best_Val_epoch = epoch

	if Valid_aacc > best_Aro_acc:
		print('Saving..')
		print("best_Val_accV: %0.3f" % Valid_vacc)
		print("best_Val_accA: %0.3f" % Valid_aacc)
		state = {
		...
		if not os.path.isdir(path):
			os.mkdir(path)
		torch.save(state, os.path.join(path,'best_aro_model.pt'))
		best_Aro_acc = Valid_aacc
		best_Aro_epoch = epoch

	if (Valid_vacc + Valid_aacc) > best_VA_acc:
		print('Saving..')
		print("best_Val_accV: %0.3f" % Valid_vacc)
		print("best_Val_accA: %0.3f" % Valid_aacc)
		state = {
		...
		if not os.path.isdir(path):
			os.mkdir(path)
		#torch.save(state, os.path.join(path,'cam_model_rec_2_v3.pt'))
		torch.save(state, os.path.join(path,'best_VA_model.pt'))
		best_VA_acc = Valid_vacc + Valid_aacc
		best_Val_acc_epoch = epoch
	epoch_toc = time.time()
